`timescale 1ns/1ps
// Exhaustive testbench for ttl_74x1g97. The input space is 3 bits (8 vectors),
// so every combination is checked, with the expected column taken verbatim
// from datasheet Table 1 (Function Table).
module tb_ttl_74x1g97;
    logic in0, in1, in2, y, expected;
    ttl_74x1g97 dut (.in0(in0), .in1(in1), .in2(in2), .y(y));
    int errors;
    ...
    initial begin
        ...
        if (errors != 0) $fatal(1, "tb_ttl_74x1g97: %0d mismatches", errors);
        $display("PASS: tb_ttl_74x1g97 (8 vectors)");
        $finish;
    end
endmodule
