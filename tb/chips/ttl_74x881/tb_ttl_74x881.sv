`timescale 1ns/1ps
// Exhaustive testbench for ttl_74x881 (4-bit ALU, '181 family). All 2^14 = 16384
...
module tb_ttl_74x881;
    logic [3:0] a, b, s;
    logic       m, cn;
    logic [3:0] f;
    logic       cn_p_4, a_eq_b, p_n, g_n;
    ttl_74x881 dut(.a(a), .b(b), .s(s), .m(m), .cn(cn), .f(f),
    ...
    initial begin
        ...
        if (errors != 0) $fatal(1, "tb_ttl_74x881: %0d mismatches", errors);
        $display("PASS: tb_ttl_74x881 (16384 vectors, exhaustive)");
        $finish;
    end
endmodule
