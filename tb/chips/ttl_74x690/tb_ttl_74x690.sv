`timescale 1ns/1ps
// Reference-model + directed testbench for ttl_74x690. A behavioral reference
...
// step, plus directed checks on the decade wrap value and the R/C and /G muxes.
module tb_ttl_74x690;
    logic       cck, cclr_n, load_n, enp, ent, rck, rclr_n, r_c, g_n;
    logic [3:0] d, q;
    logic       rco;
    ttl_74x690 dut (.cck(cck), .cclr_n(cclr_n), .load_n(load_n), .enp(enp),
    ...
    initial begin
        ...
        load_n=0; d=4'd8; ctick; load_n=1; chk("load");
        enp=1; ent=1;
        ...
        if (errors != 0) $fatal(1, "tb_ttl_74x690: %0d mismatches", errors);
        $display("PASS: tb_ttl_74x690 (directed + 1500 random)");
        $finish;
    end
endmodule
