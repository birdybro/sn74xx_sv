...
module tb_ttl_74x2236;
    ...
    initial begin
        ...
        rst_n = 0; #2; rst_n = 1; #2;
        if (ef_n_ba !== 0) errors++;
        din_ba = 9'h1FE; wr_n_ba = 0;
        #1; wclk_ba = 1; #1; wclk_ba = 0; wr_n_ba = 1; #1;
        if (ef_n_ba !== 1) errors++;
        if (dout_ba !== 9'h1FE) errors++;
        if (errors != 0) $fatal(1, "tb_ttl_74x2236: %0d errors", errors);
        $display("PASS: tb_ttl_74x2236");
        $finish;
    end
endmodule
