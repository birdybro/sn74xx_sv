`timescale 1ns/1ps
// Reference-model + directed testbench for ttl_74x699 (up/down counter + register
...
module tb_ttl_74x699;
    logic       cck, cclr_n, load_n, u_d, enp, ent, rck, r_c, g_n;
    logic [3:0] d, q;
    logic       rco;
    ttl_74x699 dut (.cck(cck), .cclr_n(cclr_n), .load_n(load_n), .u_d(u_d),
    ...
    initial begin
        ...
        if (errors != 0) $fatal(1, "tb_ttl_74x699: %0d mismatches", errors);
        $display("PASS: tb_ttl_74x699 (directed + 1500 random)");
        $finish;
    end
endmodule
