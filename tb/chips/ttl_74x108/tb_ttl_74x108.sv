`timescale 1ns/1ps
module tb_ttl_74x108;
    logic clk, clr_n;
    logic [1:0] pre_n, j, k, q, q_n;
    ttl_74x108 dut(.clk(clk), .clr_n(clr_n), .pre_n(pre_n), .j(j), .k(k), .q(q), .q_n(q_n));
    ...
    initial begin
        ...
        if (errors != 0) $fatal(1, "tb_ttl_74x108: %0d errors", errors);
        $display("PASS: tb_ttl_74x108");
        $finish;
    end
endmodule
