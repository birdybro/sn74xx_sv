`timescale 1ns/1ps
// Exhaustive testbench for ttl_74x3g58: all 512 input combinations checked
// against the verified single-gate function y = in2 ? ~in1 : in0 (SN74LVC1G58).
module tb_ttl_74x3g58;
    logic [2:0] in0, in1, in2, y, expected;
    ttl_74x3g58 dut (.in0(in0), .in1(in1), .in2(in2), .y(y));
    int errors;
    initial begin
        errors = 0;
        ...
        if (errors != 0) $fatal(1, "tb_ttl_74x3g58: %0d mismatches", errors);
        $display("PASS: tb_ttl_74x3g58 (512 vectors)");
        $finish;
    end
endmodule
