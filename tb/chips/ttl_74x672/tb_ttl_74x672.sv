`timescale 1ns/1ps
// Reference-model testbench for ttl_74x672. Event-driven reference always_ff
...
// sync clear) and the random phase soaks the device.
module tb_ttl_74x672;
    logic       ser_r, srck, ser_l, srclr_n, rck, r_s, g_n, s0, s1, casc;
    logic [3:0] d, q;
    ttl_74x672 dut(.ser_r(ser_r), .srck(srck), .d(d), .ser_l(ser_l),
        .srclr_n(srclr_n), .rck(rck), .r_s(r_s), .g_n(g_n), .s0(s0), .s1(s1),
        .q(q), .casc(casc));

    logic [3:0] sr_r, str_r;
    always_ff @(posedge srck) begin
    ...
    initial begin
        ...
        if (errors!=0) $fatal(1, "tb_ttl_74x672: %0d mismatches", errors);
        $display("PASS: tb_ttl_74x672 (directed + 1500 random)");
        $finish;
    end
endmodule
