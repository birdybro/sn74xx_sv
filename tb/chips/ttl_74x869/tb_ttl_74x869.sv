`timescale 1ns/1ps
// Reference-model + directed testbench for ttl_74x869. A behavioral model mirrors
...
// sync clear, plus 2000 randomized steps.
module tb_ttl_74x869;
    logic       clk, s0, s1, enp_n, ent_n, rco_n;
    logic [7:0] d, q;
    ttl_74x869 dut(.clk(clk), .s0(s0), .s1(s1), .enp_n(enp_n), .ent_n(ent_n),
        .d(d), .q(q), .rco_n(rco_n));
    ...
    task automatic chk(input string t);
        #1;
        // clear is synchronous (handled on the clock edge)
    ...
    initial begin
        ...
        if (errors!=0) $fatal(1, "tb_ttl_74x869: %0d mismatches", errors);
        $display("PASS: tb_ttl_74x869 (directed + 2000 random)");
        $finish;
    end
endmodule
