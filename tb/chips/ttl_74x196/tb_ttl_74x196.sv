`timescale 1ns/1ps
module tb_ttl_74x196;
    logic clk_a, clk_b, clr_n, load_n;
    logic [3:0] d;
    logic q_a, q_b, q_c, q_d;
    ttl_74x196 dut(.clk_a(clk_a), .clk_b(clk_b), .clr_n(clr_n), .load_n(load_n), .d(d), .q_a(q_a), .q_b(q_b), .q_c(q_c), .q_d(q_d));
    ...
    initial begin
        ...
        load_n=0; d=4'b1001; #1;
        if ({q_d,q_c,q_b,q_a} !== 4'b1001) errors++;
        if (errors != 0) $fatal(1, "tb_ttl_74x196: %0d errors", errors);
        $display("PASS: tb_ttl_74x196");
        $finish;
    end
endmodule
