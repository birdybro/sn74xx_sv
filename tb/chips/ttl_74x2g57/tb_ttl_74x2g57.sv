`timescale 1ns/1ps
// Exhaustive testbench for ttl_74x2g57: all 64 input combinations checked
// against the verified single-gate function y = in2 ? in1 : ~in0 (SN74LVC1G57).
module tb_ttl_74x2g57;
    logic [1:0] in0, in1, in2, y, expected;
    ttl_74x2g57 dut (.in0(in0), .in1(in1), .in2(in2), .y(y));
    int errors;
    initial begin
        errors = 0;
        ...
        if (errors != 0) $fatal(1, "tb_ttl_74x2g57: %0d mismatches", errors);
        $display("PASS: tb_ttl_74x2g57 (64 vectors)");
        $finish;
    end
endmodule
