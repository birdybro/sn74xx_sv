`timescale 1ns/1ps
module tb_ttl_74x115;
    logic [1:0] clk, j, k, clr_n, q, q_n;
    ttl_74x115 dut(.clk(clk), .j(j), .k(k), .clr_n(clr_n), .q(q), .q_n(q_n));
    ...
    initial begin
        ...
        if (errors != 0) $fatal(1, "tb_ttl_74x115: %0d errors", errors);
        $display("PASS: tb_ttl_74x115");
        $finish;
    end
endmodule
