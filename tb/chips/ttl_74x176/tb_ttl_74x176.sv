`timescale 1ns/1ps
module tb_ttl_74x176;
    logic clk_a, clk_b, clr_n, load_n;
    logic [3:0] d;
    logic q_a, q_b, q_c, q_d;
    ttl_74x176 dut(.clk_a(clk_a), .clk_b(clk_b), .clr_n(clr_n), .load_n(load_n),
    ...
    initial begin
        ...
        load_n=1;
        if (errors != 0) $fatal(1, "tb_ttl_74x176: %0d errors", errors);
        $display("PASS: tb_ttl_74x176");
        $finish;
    end
endmodule
