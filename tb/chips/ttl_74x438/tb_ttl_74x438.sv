`timescale 1ns/1ps
module tb_ttl_74x438;
    logic [7:0] status;
    logic dbin, wr_n, hlda, mem_r_n, mem_w_n, io_r_n, io_w_n, inta_n;
    ttl_74x438 dut(.status(status), .dbin(dbin), .wr_n(wr_n), .hlda(hlda),
    ...
    initial begin
        ...
        if (mem_r_n !== 0) errors++;
        if (errors != 0) $fatal(1, "tb_ttl_74x438: %0d errors", errors);
        $display("PASS: tb_ttl_74x438");
        $finish;
    end
endmodule
