...
- Notes:
  - A->B non-inverting, B->A inverting; Verilator lint+sim clean (yosys not installed locally)
...
- Notes:
  - OC variant of 74x643; A->B non-inverting, B->A inverting; Verilator lint+sim clean
...
- Notes:
  - Inverting variant of 74x646; Verilator lint+sim clean
...
- Notes:
  - OC twin of 74x648; Verilator lint+sim clean
...
- Notes:
  - Inverting registered transceiver; gab + gba_n dual-enable; Verilator lint+sim clean
...
- Notes:
  - Datasheet not yet accessible via current sources; specific control-pin layout (shift register variants 671-676, ALS-family address comparators 677-680, accumulator 681) needs confirmation before RTL
  - 4-bit universal shift register + storage register + R/S mux + cascade out, SYNCHRONOUS SR clear (SN74LS672, D2638). Reference-model tb (directed + 1500 random); lint clean.
...
- Notes:
  - Datasheet not yet accessible via current sources; specific control-pin layout (shift register variants 671-676, ALS-family address comparators 677-680, accumulator 681) needs confirmation before RTL
  - 16-bit parallel-in serial-out shift register, 4 modes (hold/write/read-recirculate/load), falling-edge clk, 3-state SER/Q15 (split si/q15) (SN74LS674, SDLS195). Reference-model tb (directed + 1500 random); lint clean.
...
- Notes:
  - Datasheet not yet accessible via current sources; specific control-pin layout (shift register variants 671-676, ALS-family address comparators 677-680, accumulator 681) needs confirmation before RTL
  - 16-bit address comparator: P=k threshold; Y low iff enabled and A1..Ak low, A(k+1)..A16 high (SN74ALS677A function table, all 16 rows). 64 directed + 1000 random vectors; lint clean.
...
- Notes:
  - Datasheet not yet accessible via current sources; specific control-pin layout (shift register variants 671-676, ALS-family address comparators 677-680, accumulator 681) needs confirmation before RTL
  - 16-bit address comparator with transparent output latch (C high=transparent y=~match, C low=hold). Compare core same as '677. always_latch. Directed + latch-hold sequences + 2000 random; lint clean.
...
- Notes:
  - Datasheet not yet accessible via current sources; specific control-pin layout (shift register variants 671-676, ALS-family address comparators 677-680, accumulator 681) needs confirmation before RTL
  - 12-bit address comparator with transparent latch (datasheet '680 = latch, not enable; chips.json description reversed). Compare core same as '677, 12 A inputs; C high=transparent, C low=hold. always_latch. Directed + hold sequences + 2000 random; lint clean.
...
- Notes:
  - OC twin of 74x684; Verilator lint+sim clean
...
- Notes:
  - Separate G1 (P=Q) and G2 (P>Q) active-low enables; Verilator lint+sim clean
...
- Notes:
  - OC twin of 74x686; Verilator lint+sim clean
...
- Notes:
  - Counter/latch/multiplexer family datasheet not accessible from current sources; needs confirmation of control-pin set (load, count, latch, mux-select, OE) before RTL
  - 4-bit decade counter + snapshot register + R/C mux + /G 3-state, SYNCHRONOUS counter clear (SN74LS692, D2423; datasheet 'register', chips.json 'latch'). Reference-model tb + 1500 random; lint clean.
...
- Notes:
  - Counter/latch/multiplexer family datasheet not accessible from current sources; needs confirmation of control-pin set (load, count, latch, mux-select, OE) before RTL
  - 4-bit binary counter + snapshot register + R/C mux + /G 3-state, SYNCHRONOUS counter clear (SN74LS693, D2423; datasheet 'register', chips.json 'latch'). Reference-model tb + 1500 random; lint clean.
...
- Notes:
  - Counter/latch/multiplexer family datasheet not accessible from current sources; needs confirmation of control-pin set (load, count, latch, mux-select, OE) before RTL
  - 4-bit decade up/down counter + register + R/C mux + /G, SYNCHRONOUS clear (SN74LS698, D2424). Reference-model tb + 1500 random; lint clean.
...
- Notes:
  - Counter/latch/multiplexer family datasheet not accessible from current sources; needs confirmation of control-pin set (load, count, latch, mux-select, OE) before RTL
  - 4-bit binary up/down counter + register + R/C mux + /G, SYNCHRONOUS clear (SN74LS699, D2424). Reference-model tb + 1500 random; lint clean.
...
- Notes:
  - Dual 4-bit inverting line driver; functional twin of 74x240; Verilator lint+sim clean
...
- Notes:
  - Specialized cache comparator / NuBus / GTL / clock driver / FIFO / level translator — needs dedicated datasheet
  - dual 256x1 FIFO (two independent queues) via gen_fifo.py; posedge write/read, async clear, full/empty flags. Testbench passes; lint clean.
...
- Notes:
  - Specialized cache comparator / NuBus / GTL / clock driver / FIFO / level translator — needs dedicated datasheet
  - 64x8 synchronous FIFO via gen_fifo.py; posedge write/read, async clear, full/empty flags. Testbench passes; lint clean.
...
- Notes:
  - Behavioral bidirectional FIFO (two 32x9 queues). Verilator + Yosys clean.
...
- Notes:
  - Logically identical to 74x640; MOS drive strength not modeled. Verilator + Yosys clean.
...
- Notes:
  - Logically identical to 74x643; MOS drive strength not modeled. Verilator + Yosys clean.
...
- Notes:
  - Identical to 74x648. Verilator + Yosys clean.
...
- Notes:
  - Identical to 74x651. Verilator + Yosys clean.
...
- Notes:
  - 9-bit D-FF with async clear and 3-state output. Verilator + Yosys clean.
...
- Notes:
  - Logically identical to 74x833. Verilator + Yosys clean.
...
- Notes:
  - 9-bit non-inverting bus transceiver with independent A/B output enables. Verilator + Yosys clean.
...
- Notes:
  - Bidir FIFO (2x64x36) via gen_fifo.py --bidir. Verilator + Yosys clean.
...
- Notes:
  - Generated via scripts/gen_fifo.py (64x36); same as 74x3611. Verilator + Yosys clean.
...
- Notes:
  - Bidir FIFO (2x256x36) via gen_fifo.py --bidir. Verilator + Yosys clean.
...
- Notes:
  - Bidir FIFO (2x512x32) via gen_fifo.py --bidir. Verilator + Yosys clean.
...
- Notes:
  - Generated via scripts/gen_fifo.py (1024x36). Verilator + Yosys clean.
...
- Notes:
  - Bidir FIFO (2x1024x36) via gen_fifo.py --bidir. Verilator + Yosys clean.
...
- Notes:
  - 12-bit ripple binary counter. Verilator + Yosys clean.
...
- Notes:
  - 2-digit BCD presettable sync down counter with TC output. Verilator + Yosys clean.
...
- Notes:
  - Quad 2-input NOR, = 74x02. Verilator + Yosys clean.
...
- Notes:
  - Quad 2-input NAND, = 74x00. Verilator + Yosys clean.
...
- Notes:
  - Mixed gate package: 2 inverters + various NAND/NOR. Verilator + Yosys clean.
...
- Notes:
  - Mixed gate package: 2 inverters, 3 NAND, 3 NOR. Verilator + Yosys clean.
...
- Notes:
  - 512x9 FIFO via gen_fifo.py. Verilator + Yosys clean.
...
- Notes:
  - 1024x9 FIFO via gen_fifo.py. Verilator + Yosys clean.
...
- Notes:
  - Dual inverters; unbuffered CMOS not modeled. Verilator + Yosys clean.
...
- Notes:
  - 8192x9 FIFO via gen_fifo.py. Verilator + Yosys clean.
...
- Notes:
  - 16384x9 FIFO via gen_fifo.py. Verilator + Yosys clean.
...
- Notes:
  - Octal inverting buffer, = 74x240. Verilator + Yosys clean.
...
- Notes:
  - 64x4 FIFO via gen_fifo.py. Verilator + Yosys clean.
...
- Notes:
  - 512x18 FIFO. Verilator + Yosys clean.
...
- Notes:
  - 64x18 FIFO. Verilator + Yosys clean.
...
- Notes:
  - Bidir 2x64x36 FIFO. Verilator + Yosys clean.
...
- Notes:
  - 64x36 FIFO. Verilator + Yosys clean.
...
- Notes:
  - Bidir 2x512x18 FIFO. Verilator + Yosys clean.
...
- Notes:
  - Bidir 2x512x32 FIFO. Verilator + Yosys clean.
...
- Notes:
  - 1024x36 FIFO. Verilator + Yosys clean.
...
- Notes:
  - 2048x18 FIFO. Verilator + Yosys clean.
...
- Notes:
  - = 74x374 with JTAG (not modeled). Verilator + Yosys clean.
...
- Notes:
  - = 74x646 with JTAG (not modeled). Verilator + Yosys clean.
...
- Notes:
  - 8-bit inverting latched bidir transceiver. Verilator + Yosys clean.
...
- Notes:
  - 8-bit non-inverting latched bidir transceiver. Verilator + Yosys clean.
...
- Notes:
  - 9-bit inverting latched bidir transceiver. Verilator + Yosys clean.
...
- Notes:
  - 9-bit buffer. Verilator + Yosys clean.
...
- Notes:
  - 8-bit universal shift register (hold/SR/SL/load). Verilator + Yosys clean.
...
- Notes:
  - 16-bit version of 74x245. Verilator + Yosys clean.
...
- Notes:
  - Same as 74x16620. Verilator + Yosys clean.
...
- Notes:
  - 36-bit version of 74x245. Verilator + Yosys clean.
...
- Notes:
  - 36-bit version of 74x543. Verilator + Yosys clean.
...
- Notes:
  - 16-bit transceiver + 16-bit latch (= 16973 widened). Verilator + Yosys clean.
...
- Notes:
  - 32-bit version of 74x244. Verilator + Yosys clean.
...
- Notes:
  - Single 2-input NOR. Verilator + Yosys clean.
...
- Notes:
  - Single 2-input AND (open-drain not modeled). Verilator + Yosys clean.
...
- Notes:
  - Single 3-input NAND. Verilator + Yosys clean.
...
- Notes:
  - Single 1-of-2 demux. Verilator + Yosys clean.
...
- Notes:
  - Single 1-to-2 decoder, active-low. Verilator + Yosys clean.
...
- Notes:
  - Single 2-input OR. Verilator + Yosys clean.
...
- Notes:
  - Single buffer. Verilator + Yosys clean.
...
- Notes:
  - Dual 2-input NAND via gen_simple_gate.py. Verilator + Yosys clean.
...
- Notes:
  - Dual 2-input NOR. Verilator + Yosys clean.
...
- Notes:
  - Dual inverter. Verilator + Yosys clean.
...
- Notes:
  - Dual unbuffered inverter. Verilator + Yosys clean.
...
- Notes:
  - Dual inverter (open-drain not modeled). Verilator + Yosys clean.
...
- Notes:
  - Dual buffer (open-drain not modeled). Verilator + Yosys clean.
...
- Notes:
  - Dual buffer. Verilator + Yosys clean.
...
- Notes:
  - Dual 2-input NAND (open-drain). Verilator + Yosys clean.
...
- Notes:
  - Dual D-FF, Q only. Verilator + Yosys clean.
...
- Notes:
  - Dual inverting buffer with OE_n. Verilator + Yosys clean.
...
- Notes:
  - Triple inverter. Verilator + Yosys clean.
...
- Notes:
  - Triple unbuffered inverter. Verilator + Yosys clean.
...
- Notes:
  - Triple inverter (open-drain). Verilator + Yosys clean.
...
- Notes:
  - Single-supply 2-input NOR. Verilator + Yosys clean.
...
- Notes:
  - Single-supply inverter. Verilator + Yosys clean.
...
- Notes:
  - Configurable 9-function gate; requires datasheet
  - AUP single-supply translator (digital pass-through). Logic = '1G58: y = c ? ~b : a (SN74AUP1T58). Exhaustive 8-vector tb; lint clean.
...
- Notes:
  - Dual-supply 4-bit bus xceiver. Verilator + Yosys clean.
...
- Notes:
  - Dual-supply 20-bit bus xceiver. Verilator + Yosys clean.
...
- Notes:
  - Dual-supply 24-bit bus xceiver. Verilator + Yosys clean.
...
- Notes:
  - Dual-supply 32-bit bus xceiver. Verilator + Yosys clean.
