# ttl_74x696 — 4-bit decimal up/down counter/register/multiplexer, asynchronous clear

4-bit decade synchronous **up/down** counter with a snapshot register and
...
- **Counter** (clock `cck`): asynchronous (direct) clear via `cclr_n`; synchronous parallel
  load from `d` when `load_n` low; counts when `enp & ent` — `u_d`=1 up,
  `u_d`=0 down; decade wrap (up `9 -> 0`, down `0 -> 9`).
...
decade wraps, `rco`, load, hold, register capture/hold, `R/C`, `/G`, and the
async counter clear, plus 1500 randomized steps. Verilator `--lint-only
-Wall` clean.
