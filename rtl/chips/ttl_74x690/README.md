# ttl_74x690 — 4-bit decimal counter/register/multiplexer, asynchronous clear

4-bit decade synchronous counter with a snapshot register and multiplexed
3-state outputs. Datasheet: TI **SN54LS690-693** (D2423).

## Function

- **Counter** (clock `cck`): asynchronous clear via `cclr_n`; synchronous parallel
  load from `d` when `load_n` low; counts up when `enp & ent`; decade count
  wraps `9 -> 0`. `rco = ent & (count == 9)` for cascading.
...
checks cover load, the decade wrap and `rco`, hold, register capture/hold,
`R/C` select, `/G`, both clears (including clear asserted between clock edges to
exercise the async counter-clear), plus 1500 randomized steps. Verilator
